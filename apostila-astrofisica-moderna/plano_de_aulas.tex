...
\title{\textbf{Astrofísica Moderna}\\Plano de aulas e fichas de estudo\\49 aulas}
...
{\large\color{astroMuted} 49 aulas\par}
\vfill
\begin{noticebox}
...
A leitura conceitual completa está na apostila principal, \texttt{apostila\_astrofisica\_moderna.tex}. Aqui, a função das fichas é organizar o ritmo do curso e indicar o que preparar, revisar e praticar em cada encontro.

\section*{Distribuição das 49 aulas}
\addcontentsline{toc}{section}{Distribuição das 49 aulas}
\begin{longtable}{>{\raggedright\arraybackslash}p{1.2cm}>{\raggedright\arraybackslash}p{9.2cm}>{\raggedright\arraybackslash}p{3.2cm}}
\toprule
Aulas & Tema & Bloco \\
\midrule
1--7 & Ferramentas observacionais: céu, órbitas, luz e telescópios & Fundamentos \\
8--22 & Estrelas: parâmetros, espectros, interiores, Sol, formação e evolução & Astrofísica estelar \\
23--26 & Compactos, relatividade e binárias interagentes & Altas energias \\
27--35 & Sistema Solar, corpos menores e exoplanetas & Astrofísica planetária \\
36--43 & Via Láctea, galáxias, evolução galáctica e estrutura cósmica & Galáxias \\
44--49 & Galáxias ativas, cosmologia, CMB, energia escura e universo primordial & Cosmologia \\
\bottomrule
\end{longtable}
...
\textbf{Leitura de apoio:} Capítulo 1 da apostila, completo, mais o laboratório do capítulo --- \textbf{o céu de Joinville}, em que azimute e altura medidos viram coordenadas equatoriais e, por ajuste, a inclinação do plano galáctico e a obliquidade da eclíptica.
\end{lessonblock}
\atividade{Desenhe a esfera celeste local: horizonte, zênite, polos celestes e equador celeste para a latitude de sua cidade. Depois abra o laboratório e registre meia dúzia de objetos, conferindo uma das conversões à mão.}
\exercicios{
...
\aula{2}{Paralaxe, brilho e luminosidade}{cap. 1}
\objetivos{Medir distância por geometria pura e converter brilho aparente em luminosidade, declarando as hipóteses de cada passo.}
\ideias{
\item A paralaxe é geometria pura, $d\,[\mathrm{pc}]=1/p\,['']$, e o parsec é definido exatamente para que essa inversão valha. O trânsito de Vênus foi o método clássico para fixar a UA em quilômetros.
\item Medir uma paralaxe em uma imagem é medir um deslocamento em pixels e convertê-lo pela escala da imagem --- e dividir por dois, porque janeiro a julho cobre a elipse inteira.
\item O erro relativo passa intacto da paralaxe para a distância: $\sigma_d/d=\sigma_p/p$. Acima de $20\%$ a inversão deixa de ser legítima.
\item Fluxo e luminosidade se ligam pela lei do inverso do quadrado, $F=L/4\pi d^{2}$, sob duas hipóteses que podem falhar: emissão isotrópica e meio transparente.
...
\item A cor é independente da distância, e por isso mede temperatura sem exigir que se saiba onde o objeto está --- mas só na ausência de poeira.
}
...
Tempo & Atividade \\
\midrule
0--10 min & De onde vem 1 UA em quilômetros: o trânsito de Vênus e a geometria que o converte em escala. \\
10--22 min & Paralaxe: geometria, definição do parsec, ordem de grandeza dos ângulos. Medir uma paralaxe em uma imagem, e o fator 2. \\
22--30 min & Propagação da incerteza, $\sigma_d/d=\sigma_p/p$, e quando inverter a paralaxe deixa de valer. \\
30--42 min & Fluxo, luminosidade e lei do inverso do quadrado; dedução da escala de Pogson e do módulo de distância. \\
42--50 min & Cores, extinção e avermelhamento: $A_\lambda=1{,}086\,\tau_\lambda$, excesso de cor e $R_V$. Fechamento com a cadeia observável $\rightarrow$ grandeza física. \\
\bottomrule
\end{tabular}

\textbf{Leitura de apoio:} Capítulo 2 da apostila, completo, mais o Laboratório de dados 1. \textbf{É este capítulo que o primeiro artigo do semestre cobra.}
\end{lessonblock}
\atividade{Abrir o laboratório do Capítulo 2 e medir a paralaxe de uma estrela na régua, conferindo o resultado com a coluna do catálogo.}
\exercicios{
\item Por que o deslocamento entre janeiro e julho vale duas paralaxes, e não uma?
\item Duas estrelas têm a mesma magnitude aparente e distâncias na razão 1:10. Qual a razão entre as luminosidades?
}

\aula{3}{Movimentos, unidades e fotometria}{cap. 1}
\objetivos{Separar paralaxe dos outros dois efeitos anuais de posição e entender o que virou exato por definição nas unidades astronômicas.}
\ideias{
\item Aberração ($20{,}5''$) e paralaxe (menos de $1''$) são efeitos distintos, defasados de um quarto de período, e só o segundo depende da distância. A constante de aberração é quase trinta vezes a maior paralaxe do céu.
...
\item O fator $4{,}74$ não é constante física: é a razão entre unidades escolhidas por seres humanos --- 1 UA por ano, em km/s.
\item Desde 2012 a UA é exata por convenção, e o parsec é consequência dela. Adota-se $GM_\odot$, e não $M_\odot$, porque o produto é medido com muito mais algarismos que $G$.
\item Inverter a paralaxe é enviesado quando $\sigma_p/p\gtrsim0{,}2$, por efeito de volume --- o viés de Lutz-Kelker.
\item Medir em uma banda nunca é medir a luminosidade: a correção bolométrica corrige a fração do fluxo que o filtro deixa passar, e depende quase inteiramente da temperatura efetiva.
}
...
Tempo & Atividade \\
\midrule
0--12 min & Aberração e movimento próprio: os dois efeitos que se confundem com paralaxe, e como a defasagem os separa. \\
12--24 min & Velocidade tangencial e espacial; dedução do fator $4{,}74$; a estrela de Barnard como caso extremo. \\
24--34 min & As unidades modernas: o que virou exato por definição, e por que se adota $GM_\odot$. \\
34--44 min & O viés de Lutz-Kelker, com populações sintéticas; a regra prática de $\sigma_p/p$. \\
44--50 min & Correção bolométrica e o zero da escala fixado em 2015; sistemas fotométricos, Vega contra AB. \\
\bottomrule
\end{tabular}

\textbf{Leitura de apoio:} Capítulo 3 da apostila. Nada aqui é pré-requisito para o artigo --- o que o artigo exige está no Capítulo 2.
\end{lessonblock}
\atividade{Abrir o arquivo de 2000 estrelas do Gaia e localizar a de maior movimento próprio; conferir a velocidade tangencial obtida.}
\exercicios{
\item Por que a aberração não depende da distância da estrela?
\item Uma estrela tem $\mu=2''$/ano e $p=100$ mas. Qual a sua velocidade tangencial?
}

\aula{4}{Órbitas elípticas e leis de Kepler}{cap. 2}
...
\aula{5}{Gravitação newtoniana e teorema do virial}{cap. 2}
...
\aula{6}{Luz, magnitudes e corpo negro}{cap. 3}
...
\aula{7}{Fótons, espectros e telescópios}{caps. 5 e 6}
...
\aula{8}{Sistemas binários e massas estelares}{cap. 7}
...
\aula{9}{Classificação espectral e diagrama HR}{cap. 8}
...
\aula{10}{Atmosferas estelares e opacidade}{cap. 9}
...
\aula{11}{Equilíbrio hidrostático}{cap. 10}
...
\aula{12}{Fontes de energia das estrelas}{cap. 10}
...
\aula{13}{Transporte de energia no interior estelar}{cap. 10}
...
\aula{14}{Modelos estelares e sequência principal}{cap. 10}
...
\aula{15}{O Sol como estrela padrão}{cap. 11}
...
\aula{16}{Meio interestelar: gás e poeira}{cap. 12}
...
\aula{17}{Formação estelar e protoestrelas}{cap. 12}
...
\aula{18}{Evolução na sequência principal}{cap. 13}
...
\aula{19}{Gigantes, queima de hélio e estágios tardios}{cap. 13}
...
\aula{20}{Pulsação estelar e sismologia}{cap. 14}
...
\aula{21}{Estrelas massivas e supernovas}{cap. 15}
...
\aula{22}{Anãs brancas e matéria degenerada}{cap. 16}
...
\aula{23}{Estrelas de nêutrons e pulsares}{cap. 16}
...
\aula{24}{Relatividade geral e buracos negros}{cap. 17}
...
\aula{25}{Sistemas binários próximos e acreção}{cap. 18}
...
\aula{26}{Síntese da astrofísica estelar}{caps. 7--18}
...
\aula{27}{Processos físicos no Sistema Solar}{cap. 19}
...
\aula{28}{Planetas terrestres}{cap. 20}
...
\aula{29}{Gigantes gasosos, luas e anéis}{cap. 21}
...
\aula{30}{Corpos menores: asteroides, cometas e cinturão de Kuiper}{cap. 22}
...
\aula{31}{Formação do Sistema Solar}{cap. 23}
...
\aula{32}{Exoplanetas: métodos de detecção}{caps. 7 e 23}
...
\aula{33}{Arquiteturas planetárias e habitabilidade}{cap. 23}
...
\aula{34}{Escalas, populações e inventário planetário}{caps. 19--23}
...
\aula{35}{Revisão aplicada: do céu local aos sistemas planetários}{caps. 1--23}
...
\aula{36}{A Via Láctea: estrutura geral}{cap. 24}
...
\aula{37}{Cinemática galáctica e matéria escura}{cap. 24}
...
\aula{38}{O centro galáctico}{cap. 24}
...
\aula{39}{Tipos de galáxias e sequência de Hubble}{cap. 25}
...
\aula{40}{Estrutura espiral e formação estelar em galáxias}{cap. 25}
...
\aula{41}{Galáxias elípticas, interações e fusões}{caps. 25 e 26}
...
\aula{42}{Escada de distâncias extragalácticas}{cap. 27}
...
\aula{43}{Expansão do universo e grandes estruturas}{cap. 27}
...
\aula{44}{Galáxias ativas e quasares}{cap. 28}
...
\aula{45}{Cosmologia newtoniana e parâmetros cósmicos}{cap. 29}
...
\aula{46}{Radiação cósmica de fundo}{cap. 29}
...
\aula{47}{Cosmologia relativística e energia escura}{cap. 29}
...
\aula{48}{Universo primordial e inflação}{cap. 30}
...
\aula{49}{Síntese final: uma narrativa física do universo}{todo o livro}
