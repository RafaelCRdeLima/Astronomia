...
Neste laboratório você atua como um observador trabalhando com imagens reais do Sloan Digital Sky Survey (SDSS). O painel mostra recortes de campos observados nos filtros $g$, $r$ e $i$. A tarefa não é receber a temperatura pronta: primeiro você deve escolher uma estrela, desenhar uma abertura sobre o alvo, desenhar uma abertura de céu em uma região de fundo, medir os fluxos líquidos em cada filtro e então transformar esses fluxos em cores, temperatura aproximada e incerteza.

\begin{lessonblock}[borderline west={2pt}{0pt}{astroGold}]{Roteiro de experimentos}
\textbf{Experimento 1: imagem científica em filtros.}
Escolha um dos campos SDSS e alterne entre os filtros $g$, $r$ e $i$. Selecione uma estrela bem isolada. Anote qualitativamente em qual filtro ela parece mais brilhante. Essa comparação visual é apenas uma hipótese inicial: a resposta quantitativa deve vir da fotometria de abertura.

\textbf{Experimento 2: fotometria de abertura.}
Com a ferramenta de abertura da estrela, circule o alvo. Com a ferramenta de fundo, circule uma região próxima sem estrela brilhante. Para cada filtro, o laboratório calcula
\[
F_{\rm estrela}=C_{\rm ap}-\frac{A_{\rm ap}}{A_{\rm fundo}}C_{\rm fundo},
\]
em que $C_{\rm ap}$ é a soma dos pixels na abertura da estrela, $C_{\rm fundo}$ é a soma dos pixels na abertura de céu e $A_{\rm ap}/A_{\rm fundo}$ corrige a diferença de áreas. Registre $F_g$, $F_r$ e $F_i$ para a estrela escolhida.

\textbf{Experimento 3: escolha do raio.}
Meça a mesma estrela com três raios de abertura: pequeno, intermediário e grande. Mantenha a mesma abertura de fundo. Faça uma tabela com $F_g$, $F_r$, $F_i$, $g-r$ e $r-i$ para os três casos. Explique numericamente por que uma abertura pequena perde fluxo e por que uma abertura grande pode aumentar o ruído.

\textbf{Experimento 4: fundo real.}
Mantendo a mesma abertura da estrela, escolha primeiro uma região de fundo limpa e depois uma região contaminada por estrelas fracas ou por fundo mais irregular. Compare os fluxos líquidos. Calcule a diferença percentual em $F_r$:
\[
\frac{\Delta F_r}{F_{r,\rm limpo}}=
\frac{|F_{r,\rm contaminado}-F_{r,\rm limpo}|}{F_{r,\rm limpo}}.
\]
Interprete: o erro veio do alvo ou da escolha do céu?

\textbf{Experimento 5: índices de cor.}
Para pelo menos cinco estrelas, registre uma medida no caderno de dados. Usando os fluxos exportados, calcule
\[
\begin{aligned}
g-r&=-2{,}5\log_{10}(F_g/F_r),\\
r-i&=-2{,}5\log_{10}(F_r/F_i).
\end{aligned}
\]
Ordene as estrelas por $g-r$. Quais são relativamente mais azuis? Quais são relativamente mais vermelhas?

\textbf{Experimento 6: temperatura aproximada.}
O painel estima uma temperatura a partir de uma conversão aproximada entre $g-r$ e $B-V$, seguida de uma relação empírica de cor--temperatura. Use os valores exportados para comparar suas estrelas. A temperatura maior corresponde sempre ao menor $g-r$? Discuta possíveis desvios: ruído, abertura, fundo, saturação e diferenças entre uma estrela real e um corpo negro ideal.

\textbf{Experimento 7: incerteza e sinal-ruído.}
Use os valores de S/N em $g$, $r$ e $i$. Para uma estrela brilhante e uma estrela fraca, compare em qual filtro a medida é mais confiável. Explique usando a dispersão do fundo: uma medida com fluxo alto, mas fundo muito variável, pode ser menos precisa do que parece.

\textbf{Experimento 8: interpretação física.}
Escolha duas estrelas com cores bem diferentes. Para cada uma, entregue $F_g$, $F_r$, $F_i$, $g-r$, $r-i$, $T$ aproximada e S/N. Escreva uma interpretação curta separando três níveis:
\[
\text{pixels medidos} \;\longrightarrow\; \text{fluxos e cores} \;\longrightarrow\; \text{temperatura inferida}.
\]
Essa separação é essencial: o detector mede contagens; temperatura é uma inferência baseada em modelo.
\end{lessonblock}

\begin{lessonblock}{Formato da entrega}
A entrega deve conter o CSV exportado pelo laboratório, uma tabela com pelo menos cinco estrelas, os raios de abertura usados, os cálculos de $g-r$ e $r-i$, a temperatura aproximada indicada ou recalculada, uma comparação entre duas escolhas de fundo e uma interpretação curta sobre o que é observação direta e o que é inferência física.
\end{lessonblock}

\begin{lessonblock}{Observação sobre os dados}
Os campos usados no painel vêm de imagens reais reduzidas do SDSS DR12. Eles não são desenhos sintéticos: há fundo, ruído, estrelas fracas, pixels ruins e pequenas imperfeições de alinhamento entre filtros. Esses detalhes fazem parte do exercício, pois forçam escolhas semelhantes às de uma redução fotométrica real.
\end{lessonblock}
\newpage
