...
\begin{noticebox}
\textbf{Como este capítulo se divide.} O conteúdo foi organizado para dois encontros de $50$ minutos.

\textbf{Aula 1 --- a geometria do céu e os sistemas de referência.} Ângulos e ângulo sólido; a esfera celeste e os três sistemas de coordenadas; trigonometria esférica e a equação da altura; eclíptica, obliquidade e estações; a equação do tempo; tempo sideral; precessão, nutação e o conceito de época; os sistemas de referência ICRS e Gaia-CRF; a cadeia de reduções que leva de uma posição de catálogo à posição aparente no telescópio.

\textbf{Aula 2 --- distâncias, brilhos e a ponte para a física.} Paralaxe e a definição do parsec; as unidades astronômicas e suas definições modernas; movimento próprio, velocidade tangencial e velocidade espacial; o viés estatístico das paralaxes; fluxo, luminosidade e a lei do inverso do quadrado; a escala de magnitudes deduzida; módulo de distância; cores e sistemas fotométricos; extinção e avermelhamento; correção bolométrica.

As seções de instrumentação --- redução de imagens, astrometria de placa e refração --- são leitura complementar: elas não cabem em duas aulas expositivas, mas são necessárias para os laboratórios.
\end{noticebox}
