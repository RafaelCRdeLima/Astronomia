...
Neste laboratório você atua como um observador que obtém espectros de estrelas desconhecidas. O painel não entrega diretamente temperatura ou velocidade: ele mostra contagens, resposta instrumental, espectro calibrado, filtros e linhas. Seu trabalho é transformar dados observados em grandezas físicas.

\begin{lessonblock}[borderline west={2pt}{0pt}{astroGold}]{Roteiro de experimentos}
\textbf{Experimento 1: calibração.}
Escolha um alvo e registre uma medida no modo bruto, uma com dark subtraído e uma com dark + resposta instrumental. Para cada uma, anote $\lambda_{\max}$ e calcule $T$ por Wien. Quantifique a diferença percentual entre a temperatura bruta e a temperatura calibrada:
\[
\frac{\Delta T}{T_{\rm cal}}=\frac{|T_{\rm bruto}-T_{\rm cal}|}{T_{\rm cal}}.
\]
Explique por que a calibração altera a forma do contínuo.

\textbf{Experimento 2: temperatura por contínuo.}
...
\textbf{Experimento 3: fotometria em bandas.}
...
\textbf{Experimento 4: velocidade radial.}
...
\textbf{Experimento 5: fluxo integrado.}
...
\begin{lessonblock}{Formato da entrega}
A entrega deve conter o CSV exportado, uma tabela com pelo menos três alvos, os cálculos de $T$ por Wien, $B-V$, velocidade radial por pelo menos uma linha, uma comparação entre espectro bruto e calibrado, e uma interpretação curta respondendo: que parte da conclusão vem diretamente dos dados e que parte depende de modelo físico?
\end{lessonblock}
\newpage
