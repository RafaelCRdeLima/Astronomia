...
Neste laboratório você trabalhará com um catálogo simulado de estrelas. O objetivo não é apenas manipular controles, mas transformar dados observados em grandezas físicas usando as equações deste capítulo. Ao final, entregue uma tabela curta, os cálculos principais e uma interpretação física em texto.

\begin{lessonblock}[borderline west={2pt}{0pt}{astroGold}]{Roteiro de experimentos}
\textbf{Preparação dos dados.} Abra o laboratório, selecione uma estrela no céu simulado e use o botão \textit{Exportar CSV}. O arquivo contém paralaxe medida, erro de paralaxe, distância real simulada, fluxo relativo, magnitude aparente, magnitude absoluta inferida, cor observada, extinção $A_V$ e luminosidade. Use pelo menos 8 estrelas do catálogo para a análise; inclua obrigatoriamente a estrela selecionada no painel principal.

\textbf{Experimento 1: paralaxe e distância.}
Para cada uma das 8 estrelas escolhidas, calcule a distância a partir da paralaxe:
\[
...
\]
Entregue uma tabela com $p$, $\sigma_p$, $d$ e $\sigma_d$. Identifique a estrela com maior erro relativo de distância e explique numericamente por que paralaxes pequenas tornam a distância menos confiável.
...
\textbf{Experimento 4: poeira e interpretação da cor.}
Escolha uma estrela e varie $A_V$ no controle do laboratório, mantendo distância e luminosidade fixas. Anote pelo menos quatro valores de $A_V$ e registre, para cada um, magnitude aparente, fluxo relativo e cor observada. Mostre que o objeto fica mais fraco quando $A_V$ aumenta. Interprete por que uma estrela pode parecer fria/vermelha sem que sua temperatura intrínseca tenha mudado.

\textbf{Experimento 5: lei do inverso do quadrado.}
Escolha uma estrela, fixe luminosidade e extinção, e varie a distância para $d$, $2d$ e $3d$ usando o controle do laboratório. Registre o fluxo relativo em cada caso. Compare as razões medidas com as previsões:
\[
\frac{F(2d)}{F(d)}=\frac{1}{4},\qquad
\frac{F(3d)}{F(d)}=\frac{1}{9}.
\]
Calcule os desvios percentuais entre o valor obtido no laboratório e o valor previsto. Interprete o resultado.
\end{lessonblock}
...
\item pelo menos um gráfico feito a partir do CSV exportado, por exemplo $m$ versus $d$ ou $L_{\rm inferida}$ versus $L_{\rm real}$;
...
Neste laboratório você observará um sistema orbital simulado em unidades astronômicas: distância em UA, tempo em anos e massa em massas solares. O objetivo é usar medidas de órbita para inferir massa, testar as leis de Kepler e decidir se uma trajetória é ligada ou de escape.

\begin{lessonblock}[borderline west={2pt}{0pt}{astroGold}]{Roteiro de experimentos}
\textbf{Preparação dos dados.} Abra o laboratório, escolha um sistema observado e exporte o CSV. O arquivo contém tempo, posição, raio orbital, velocidade, energia específica e taxa de área varrida. Use pelo menos três configurações diferentes: uma órbita quase circular, uma órbita elíptica e o modo de escape.

\textbf{Experimento 1: massa central pela terceira lei de Kepler.}
Para uma órbita ligada, meça o semieixo maior $a$ e o período $P$. Em unidades de UA, ano e massa solar, use:
\[
\frac{M}{M_\odot}=\frac{a^3}{P^2}.
\]
Calcule a massa central para pelo menos três valores de $a$. Compare com a massa escolhida no controle do laboratório e registre o erro percentual.

\textbf{Experimento 2: excentricidade orbital.}
Na órbita elíptica, determine o periastro $r_p$ e o apoastro $r_a$ a partir da tabela ou do gráfico $r(t)$. Calcule:
\[
e=\frac{r_a-r_p}{r_a+r_p}.
\]
Compare com o valor exibido no laboratório. Interprete fisicamente o que muda na órbita quando $e$ aumenta.

\textbf{Experimento 3: segunda lei de Kepler.}
Use a coluna $\Delta A/\Delta t$ do CSV para verificar se a taxa de área varrida permanece aproximadamente constante. Calcule a média, o maior desvio percentual em relação à média e explique por que a velocidade orbital aumenta perto do periastro.

\textbf{Experimento 4: energia orbital.}
Para três situações diferentes, use a energia específica:
\[
\epsilon=\frac{v^2}{2}-\frac{GM}{r}.
\]
Classifique cada caso como órbita ligada ($\epsilon<0$), escape parabólico aproximado ($\epsilon\simeq0$) ou trajetória aberta ($\epsilon>0$). No modo de escape, compare a velocidade inicial com $v_{\rm esc}=\sqrt{2GM/r}$.

\textbf{Experimento 5: gráfico de Kepler.}
Gere uma tabela com pelo menos cinco valores de $a$ para a mesma massa central. Para cada caso, meça $P$, calcule $a^3$ e $P^2$, e faça um gráfico $P^2$ versus $a^3$. A inclinação esperada é $1/M$ quando as unidades são UA, ano e $M_\odot$. Interprete a inclinação encontrada.
\end{lessonblock}

\begin{lessonblock}{Formato da entrega}
A entrega deve conter uma tabela com as configurações usadas, um gráfico $P^2$ versus $a^3$, um gráfico $r(t)$ de uma órbita elíptica, os cálculos de massa, excentricidade, energia e taxa de área, sempre com unidades. A conclusão deve responder: como uma órbita observada permite medir massa sem tocar no corpo central?
\end{lessonblock}
\newpage
